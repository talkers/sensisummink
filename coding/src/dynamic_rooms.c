...
dfile *dynamic_init(char *filename,int granularity)
{
  dfile *df;
  char *oldstack;
  int fd,length;
  ...
  if (sys_flags&VERBOSE) 
    vlog("sync", "Loading dynamic file '%s'",filename);
  
  df=(dfile *)MALLOC(sizeof(dfile));
  memset(df,0,sizeof(dfile));
  strcpy(df->fname,filename);
  ...
  if (fd<0) {
    vlog("error", "Failed to load old '%s' keys (creating)",df->fname);
...
/* key==0 means this is a new entry */
  ...
  else if (!convert_key(df,key,&block,&length)) return 0;
...
void dynamic_dfstats(player *p,char *str)
{
  ...
  vtell_player(p, " Total keys = %d\tTotal blocks = %d\n"
...
/* a couple of stats */
  ...
  if(p!=NULL)
     vtell_player(p, " Defragging completed\n"
                    " Old blocks = %d\n"
...
/* a couple of stats */
   ...
   if(p)
      vtell_player(p," Validation complete\n Total Rooms checked: %d\n"
                       " Rooms rejected: %d\n",rooms,rejects);
   stack = oldstack;
}
