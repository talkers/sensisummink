/* file to set up/handle/process/etc robots */
/* robot actions to be stored in robot.h */
/* robot action frequencies in config.h */
...
void init_robots(void)
{
   ...
   robots = load_file("files/stuff/robots");
   if (!robots.where)
   {
   ...
   while(*scan)
   {
      ...
      while(scan && *scan && !isalpha(*scan))
         scan++;
      if(scan && *scan) /* ie we found a name i hope */
      {
   ...
   if (robots.where) free(robots.where);
   stack = oldstack;
}
...
void		parse_move_into_ibuffer(player *p, char *str)
{
  ...
  for(scan=str; scan && *scan; scan++) { /* check it all. */
    switch(*scan) {
...
void run_action(robot *robby)
{
   ...
   else /* ie a move to a random adjacent room */
   {
...
void store_robot(player *p, char *str)
{
   ...
   while(robscan)
   {
      if(!strcasecmp(robscan->lower_name, str))
      {
         ...
         if(robby) {
            robby->flags |= TRIED_QUIT;
            quit(robby, "");
         }
         vtell_player(p, " Robot '%s' is now in storage.\n", robscan->lower_name);
         return;
      }
...
void unstore_robot(player *p, char *str)
{
   ...
   while(robscan)
   {
      if(!strcasecmp(robscan->lower_name, str))
      {
         ...
         robscan->flags &= ~STORED;
         vtell_player(p, " Robot '%s' is now out of storage.\n", robscan->lower_name);
         return;
      }
      robscan = robscan->next;
   }
...
void	process_robot_counters(void)
{
  ...
  for(rscan = robot_start; rscan; rscan = rscan->next)
    if(rscan->counter > 0) rscan->counter--;
}
...
{
   sprintf(stack, " -=> SensiSummink robots V%s enabled.\n", ROB_VERS);
   stack = strchr(stack, 0);
}
