/*
 * intercom_glue.c - a stripped version of the original from the
 * Intertalker communications server 1.1.2
 */
...
void start_intercom()
{
  ...
  sprintf(stack,"-=> %s <=- Intertalker communication server",TALKER_NAME);
  stack[MAX_TITLE+9]=0;
...
static void parse_user_command_who_in(char *str)
{
  ...
  while (scan)
	...
	{
#ifdef INTERCOM_EXT
	  sprintf(stack,"%s%s:",scan->location==intercom_room?"*":"", scan->name);
#else
...
void do_intercom_lsu(player *p, char *str)
{
  char empty_string[2];
...
{
  vtell_player(p, " Intercom version %s (%s)\n",INTERCOM_VERSION,__DATE__);
#ifdef INTERCOM_EXT
  vtell_player(p, " Intercom room enabled (%s).\n", INTERCOM_ROOM);
#else
  tell_player(p, " Intercom room disabled.\n");
#endif
  return;
}
...
void do_intercom_say(player *p,char *str)
{
  ...
  vtell_room_but(p, intercom_room, "%s %ss '%s'\n", p->name, method, str);
...
void intercom_site_move(player *p,char *str)
{
  ...
  vtell_player(p," Informing all remote talkers of the address change to "
	      "%s %s\n",site,port);
...
/* sensi additions */   
void ver_intercom_version(void)
{
  sprintf(stack, " -=> Grim's Intercom Server V%s enabled.\n", INTERCOM_VERSION);
  stack = strchr(stack, 0);
#ifdef INTERCOM_EXT
  sprintf(stack, "     Intercom room enabled at %s.\n", INTERCOM_ROOM);
#else
  strcpy(stack, "     Intercom room disabled.\n");
#endif
  stack = strchr(stack, 0);
}
