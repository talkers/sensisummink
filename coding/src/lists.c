/*
 * lists.c
 */

#include "include/config.h"

#include <stdio.h>
#include <ctype.h>
#include <string.h>
#ifndef FREEBSD228
#include <malloc.h>
#endif
#include <fcntl.h>
#include <memory.h>
#if defined( OSF ) || defined( FREEBSD228 )
#include <stdlib.h>
#endif

#ifdef SUNOS
#define TIME_DEFINES
#endif

#include "include/missing_headers.h"
#include "include/proto.h"

#define ALLOWED_FRIEND 18
#define LIST_LINES_INT 16384 /* double this for every new flag you add */

/* interns */
static char *allowed_friend[ALLOWED_FRIEND]={"prefer","pre","inform","inf","noisy","nsy","find","fin","invite","inv","beep","bep","grab","grabme","grb","key","mailblock","mbk"};
static char *list_header=" - Name -         - Nsy Ign Inf Grb Frd Bar Inv Bep Blk Key Fnd Fbk Mbk Pre\n";
char	*check_legal_entry(player *, char *, char *);
char	*list_lines(list_ent *);

flag_list       list_flags[] = {
   {"noisy", NOISY},
   {"ignore", IGNORE},
   {"inform", INFORM},
   {"grab", GRAB},
   {"grabme", GRAB},
   {"friend", FRIEND},
   {"friendblock", FRNDBLOCK},
   {"mailblock", MAILBLOCK},
   {"prefer", PREFERRED},
   {"bar", BAR},
   {"invite", INVITE},
   {"beep", BEEP},
   {"block", BLOCK},
   {"key", KEY},
   {"find", FIND},
   {"nsy", NOISY},
   {"ign", IGNORE},
   {"inf", INFORM},
   {"grb", GRAB},
   {"frd", FRIEND},
   {"inv", INVITE},
   {"bep", BEEP},
   {"blk", BLOCK},
   {"key", KEY},
   {"find", FIND},
   {"fbk", FRNDBLOCK},
   {"mbk", MAILBLOCK},
   {"pre", PREFERRED},
{0, 0}};

flag_list       show_flags[] = {
   {"echo", TAG_ECHO},
   {"echos", TAG_ECHO},
   {"room", TAG_ROOM},
   {"local", TAG_ROOM},
   {"shout", TAG_SHOUT},
   {"shouts", TAG_SHOUT},
   {"tell", TAG_PERSONAL},
   {"tells", TAG_PERSONAL},
   {"autos", TAG_AUTOS},
   {"auto", TAG_AUTOS},
{0, 0}};

char           *message_string;


/* horrible routine to return a list_scan structure after doing hideous
   checks */
list_scan	listscan(player *scan, player *search)
{
   list_scan ls;
   
   if(!scan || !search) {
     ls.l = 0;
     ls.lf = 0;
     return ls;
   }
   
   ls.l = find_list_entry(scan, search->lower_name);
#ifdef ROBOTS
   if(!ls.l && search->residency & ROBOT_PRIV)
     ls.l = find_list_entry(scan, "robots");
#endif
   if(!ls.l && search->residency & PSU)
     ls.l = find_list_entry(scan, "sus");
   if(!ls.l && search->residency==NON_RESIDENT)
     ls.l = find_list_entry(scan, "newbies");
   if(!ls.l)
     ls.l = find_list_entry(scan, "everyone");
   if(ls.l && ls.l->flags & FRIEND)
     ls.lf = find_list_entry(scan, "friends");
   else
     ls.lf = 0;
   if(ls.l && !ls.lf)
     ls.lf = ls.l;
   return ls;
}

/* version of the above to check with saved players */
list_scan	listscan_saved(saved_player *scan, player *search)
{
   list_scan ls;
   
   if(!scan || !search) {
     ls.l = 0;
     ls.lf = 0;
     return ls;
   }
   
   ls.l = fle_from_save(scan, search->lower_name);
#ifdef ROBOTS
   if(!ls.l && search->residency & ROBOT_PRIV)
     ls.l = fle_from_save(scan, "robots");
#endif
   if(!ls.l && search->residency & PSU)
     ls.l = fle_from_save(scan, "sus");
   if(!ls.l && search->residency==NON_RESIDENT)
     ls.l = fle_from_save(scan, "newbies");
   if(!ls.l)
     ls.l = fle_from_save(scan, "everyone");
   if(ls.l && ls.l->flags & FRIEND)
     ls.lf = fle_from_save(scan, "friends");
   else
     ls.lf = 0;
   if(ls.l && !ls.lf)
     ls.lf = ls.l;
   return ls;
}


/* delete and entry from someones list */
void            delete_entry(saved_player * sp, list_ent * l)
{
   list_ent       *scan;
   if (!sp)
      return;
   scan = sp->list_top;
   if (scan == l)
   {
      sp->list_top = l->next;
      FREE(l);
      return;
   }
   while (scan)
      if (scan->next == l)
      {
    scan->next = l->next;
    FREE(l);
    return;
      } else
    scan = scan->next;
   log("error", "Tried to delete list entry that wasn't there.\n");
}


/* compress list */

void            tmp_comp_list(saved_player * sp)
{
   char           *oldstack;
   list_ent       *l, *next;

   l = sp->list_top;

   oldstack = stack;
   stack = store_int(stack, 0);

   while (l)
   {
      next = l->next;
      if (!l->name[0])
      {
    log("error", "Bad list entry on compress .. auto deleted.\n");
    delete_entry(sp, l);
      } else
      {
    stack = store_string(stack, l->name);
    stack = store_int(stack, l->flags);
      }
      l = next;
   }
#ifdef OSF
    store_int(oldstack, ((long) stack - (long) oldstack));
#else
    store_int(oldstack, ((int) stack - (int) oldstack));
#endif
}

void            compress_list(saved_player * sp)
{
   char           *oldstack;
   int             length;
   list_ent       *new, *l, *next;
   if (sp->saved_flags & COMPRESSED_LIST)
      return;
   sp->saved_flags |= COMPRESSED_LIST;
   oldstack = stack;
   tmp_comp_list(sp);
#ifdef OSF
    length = (long) stack - (long) oldstack;
#else
    length = (int) stack - (int) oldstack;
#endif
   if (length == 4)
   {
      sp->list_top = 0;
      stack = oldstack;
      return;
   }
   new = (list_ent *) MALLOC(length);
   memcpy(new, oldstack, length);

   l = sp->list_top;
   while (l)
   {
      next = l->next;
      FREE(l);
      l = next;
   }
   sp->list_top = new;
   stack = oldstack;
}

/* decompress list */

void            decompress_list(saved_player * sp)
{
   list_ent       *l;
   char           *old, *end, *start, *oldstack;
   int             length;

   oldstack = stack;
   if (!(sp->saved_flags & COMPRESSED_LIST))
      return;
   sp->saved_flags &= ~COMPRESSED_LIST;

   old = (char *) sp->list_top;
   start = old;
   if (!old)
      return;
   old = get_int(&length, old);
   end = old + length - 4;
   sp->list_top = 0;
   while (old < end)
   {
      l = (list_ent *) MALLOC(sizeof(list_ent));
      old = get_string(stack, old);
      strncpy(l->name, stack, MAX_NAME - 2);
      old = get_int(&(l->flags), old);
      l->next = sp->list_top;
      sp->list_top = l;
   }
   FREE(start);
   stack = oldstack;
}



/* save list */

void            construct_list_save(saved_player * sp)
{
   int             length;
   char           *where;

   if (!(sp->saved_flags & COMPRESSED_LIST) &&
       (!find_player_absolute_quiet(sp->lower_name)))
      compress_list(sp);

   if (sp->saved_flags & COMPRESSED_LIST)
   {
      if (sp->list_top)
      {
    where = (char *) sp->list_top;
    (void) get_int(&length, where);
    memcpy(stack, where, length);
    stack += length;
      } else
    stack = store_int(stack, 4);
   } else
      tmp_comp_list(sp);
}

/* retrieve list */

char           *retrieve_list_data(saved_player * sp, char *where)
{
   int             length;
   (void) get_int(&length, where);
   if (length == 4)
      sp->list_top = 0;
   else
   {
      sp->saved_flags |= COMPRESSED_LIST;
      sp->list_top = (list_ent *) MALLOC(length);
      memcpy(sp->list_top, where, length);
   }
   where += length;
   return where;
}

/* general routine to output a flag field */

char           *bit_string(int flags)
{
   static char     out[33];
   int             i;
   for (i = 0; i < 32; i++, flags >>= 1)
      if (flags & 1)
    out[i] = '*';
      else
    out[i] = ' ';
   out[32] = 0;
   return out;
}


/* count list entries */

int             count_list(player * p)
{
   list_ent       *l;
   int             count = 0;
   if (!p->saved)
      return 0;
   if (!p->saved->list_top)
      return 0;
   for (l = p->saved->list_top; l; l = l->next)
      count++;
   return count;
}

/* find list entry for a person */

list_ent       *find_list_entry(player * p, char *name)
{
   list_ent       *l;

   if (!p->saved)
      return 0;
   decompress_list(p->saved);
   l = p->saved->list_top;
   while (l)
      if (!strcasecmp(l->name, name))
         return l;
      else
         l = l->next;
   return 0;
}

/* create a list entry */

list_ent       *create_entry(player * p, char *name)
{
   list_ent       *l, *e;
   if (!p->saved)
      return 0;
   if ((count_list(p)) >= (p->max_list))
   {
      tell_player(p, " Can't create new list entry, "
                     "because your list is full.\n");
      return 0;
   }
   l = (list_ent *) MALLOC(sizeof(list_ent));
   strncpy(l->name, name, MAX_NAME - 2);
   e = find_list_entry(p, "everyone");
#ifdef ROBOTS
   if (e && strcasecmp(l->name,"sus") && strcasecmp(l->name,"newbies") && strcasecmp(l->name, "robots"))
#else
   if (e && strcasecmp(l->name,"sus") && strcasecmp(l->name,"newbies"))
#endif
      l->flags = e->flags;
   else
      l->flags = 0;
   l->next = p->saved->list_top;
   p->saved->list_top = l;
   return l;
}


/* find list entry for a person from saved file */

list_ent       *fle_from_save(saved_player * sp, char *name)
{
   list_ent       *l;
   decompress_list(sp);
   l = sp->list_top;
   while (l)
      if (!strcasecmp(l->name, name))
    return l;
      else
    l = l->next;
   return 0;
}


/* clear bits of list */

void            clear_list(player * p, char *str)
{
   char           *oldstack;
   int             count = 0;
   list_ent       *l;

   oldstack = stack;

   if (!p->saved)
   {
      tell_player(p, " You do not have a list to alter, "
                     "since you have no save file\n");
      return;
   }
   if (!*str)
   {
      tell_player(p, " Format: clist <person>\n");
      return;
   }
   while (*str)
   {
      while (*str && *str != ',' && *str != ' ')
         *stack++ = *str++;
      if (*str)
         str++;
      *stack++ = 0;
      if (*oldstack)
      {
         l = find_list_entry(p, oldstack);
         if (!l)
            vtell_player(p, " Can't find any list entry for '%s'.\n", oldstack);
         else
         {
            count++;
            vtell_player(p, " Entry removed for '%s'\n", l->name);
            delete_entry(p->saved, l);
         }
      }
      stack = oldstack;
   }
   if (!count)
      tell_player(p, " No entries removed.\n");
   else
   {
      if (count != 1)
         vtell_player(p, " Deleted %s entries.\n", number2string(count));
      else
         tell_player(p, " Deleted one entry.\n");
   }
}


/* turn a flag list into an int */
int             get_flags(player * p, char *flags, flag_list * list)
{
   int             flag = 0, new;
   char *oldstack;

   oldstack = stack;
   while (*flags && *flags == ' ')
      flags++;
   while (*flags)
   {
      while (*flags && *flags != ',')
    *stack++ = *flags++;
      if (*flags)
    flags++;
      *stack++ = 0;
      new = get_flag(list, oldstack);
      if (!new)
         vtell_player(p, " Can't find flag '%s'.\n", oldstack);
      else
         flag |= new;
      stack = oldstack;
   }
   return flag;
}


/* force change on list */
void            actual_set_list(player * p, char *str, int type)
{
   char           *flag_lst, *oldstack, msg[50], *dummyname;
   int             change, count = 0;
   list_ent       *l;
   player         *ptell;

   if (!p->saved)
   {
      tell_player(p, " You haven't got a list since you have no save file.\n");
      return;
   }
   /* what type is it? */
   oldstack = stack;
   flag_lst = next_space(str);
   if (!*flag_lst)
   {
      if(type==0)      vtell_player(p, " Format: flist <player list> <flag list>\n", p->command_used->text);
      else if(type==1) vtell_player(p, " Format: slist <player list> <flag list>\n", p->command_used->text);
      else if(type==2) vtell_player(p, " Format: rlist <player list> <flag list>\n", p->command_used->text);
      else if(type==3) vtell_player(p, " Format: tlist <player list> <flag list>\n", p->command_used->text);
      return;
   }
   *flag_lst++ = 0;
   change = get_flags(p, flag_lst, list_flags);
   if (!change)
   {
      tell_player(p, " Bad flag list, no changes made.\n");
      return;
   }
   while (*str)
   {
      while (*str && *str != ',')
         *stack++ = *str++;
      *stack++ = 0;
      if (*str)
         str++;
      if (*oldstack && (dummyname = check_legal_entry(p, oldstack, flag_lst)))
      {
         if(type==0)	  sprintf(msg, " Flag set for '%s'\n", dummyname);
         else if(type==1) sprintf(msg, " Flag set for '%s'\n", dummyname);
         else if(type==2) sprintf(msg, " Flag reset for '%s'\n", dummyname);
         else if(type==3) sprintf(msg, " Flag toggled for '%s'\n", dummyname);
         tell_player(p, msg);
         l = find_list_entry(p, dummyname);
         if (!l)
            l = create_entry(p, dummyname);
         if (l)
         {
            if(type==0)	     l->flags = change;
            else if(type==1) l->flags |= change;
            else if(type==2) l->flags &= ~change;
            else if(type==3) l->flags ^= change;
            if(!l->flags)
              delete_entry(p->saved, l);
            count++;
            if (message_string)
            {
               ptell = find_player_absolute_quiet(oldstack);
               if (ptell)
                  tell_player(ptell, message_string);
            }
         }
      }
      stack = oldstack;
   }
   if (count) {
     if(type==0)      tell_player(p, " List entries force changed.\n");
     else if(type==1) tell_player(p, " List entries set.\n");
     else if(type==2) tell_player(p, " List entries reset.\n");
     else if(type==3) tell_player(p, " List entries toggled.\n");
   }
   else
      tell_player(p, " No changes to make.\n");
}


/* player interface for above */
void		new_set_list(player *p, char *str)
{
   if(!strcmp(p->command_used->text, "flist"))
     actual_set_list(p, str, 0);
   else if(!strcmp(p->command_used->text, "slist"))
     actual_set_list(p, str, 1);
   else if(!strcmp(p->command_used->text, "rlist"))
     actual_set_list(p, str, 2);
   else if(!strcmp(p->command_used->text, "tlist"))
     actual_set_list(p, str, 3);
}


void            noisy(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: noisy <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " noisy");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s remove%s you from %s noisy list.]\n",
              p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s add%s you to %s noisy list.]\n",
              p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s add%s you to %s noisy list.]\n",
           p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   message_string = 0;
   stack = oldstack;
}


void            ignore(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: ignore <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " ignore");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s start%s listening to you again.]\n", p->name, single_s(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   else if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s ignore%s you.]\n", p->name, single_s(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s ignore%s you.]\n", p->name, single_s(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}


void            block(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: block <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " block");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s start%s listening to you again.]\n", p->name, single_s(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   else if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s block%s you off from doing tells to %s.]\n",
         p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s block%s you off from doing tells to %s]\n",
         p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

void            inform(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: inform <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " inform");
   stack = strchr(stack, 0);
   *stack++ = 0;
   if (!strcasecmp("off", str))
      actual_set_list(p, oldstack, 2);
   else if (!strcasecmp("on", str))
      actual_set_list(p, oldstack, 1);
   else
      actual_set_list(p, oldstack, 3);
   stack = oldstack;
}


void            grab(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: grab <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " grab");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s stop%s you from being able to grab %s.]\n", p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   else if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s allow%s you to grab %s.]\n", p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s allow%s you to grab %s.]\n", p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

void            friend(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: friend <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " friend");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s do%sn't like you any more.]\n", p->name, plural_es(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   else if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s make%s you %s friend.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s make%s you %s friend.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

void            bar(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: bar <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " bar");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s allow%s you back into %s rooms.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   else if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s bar%s you from %s rooms.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s bar%s you from %s rooms.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

void            invite(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: invite <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " invite");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s strike%s you off %s invite list.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   else if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s enter%s you onto %s invite list.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s enter%s you onto %s invite list.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

void            beep(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: beep <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " beep");
   stack = strchr(stack, 0);
   *stack++ = 0;
   if (!strcasecmp("off", str))
      actual_set_list(p, oldstack, 2);
   else if (!strcasecmp("on", str))
      actual_set_list(p, oldstack, 1);
   else
      actual_set_list(p, oldstack, 3);
   stack = oldstack;
}

void            key(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: key <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " key");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s snatch%s %s key back off of you.]\n", p->name, plural_es(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   else if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s present%s you with the key to %s rooms.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s present%s you with the key to %s rooms.]\n", p->name, single_s(p), gstring_possessive(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

void            listfind(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: find <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " find");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s stop%s you from seeing where %s %s while hidden.]\n",
      		p->name, single_s(p), gstring_possessive(p), isare(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   else if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s let%s you see where %s %s while hidden.]\n",
              p->name, single_s(p), gstring_possessive(p), isare(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s let%s you see where %s %s while hidden.]\n",
              p->name, single_s(p), gstring_possessive(p), isare(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

/* friendblock */
void            friendblock(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: friendblock <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " friendblock");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s start%s listening to your friend tells again.]\n", p->name, single_s(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   if (!strcasecmp("on", str)) {
      sprintf(stack, " [%s block%s you off from doing friend tells to %s.]\n",
         p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s block%s you off from doing friend tells to %s.]\n",
         p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

/* mailblock */
void            mailblock(player * p, char *str)
{
   char           *oldstack;
   oldstack = stack;

   if (!*str)
   {
      tell_player(p, " Format: mailblock <player(s)> on/off/[blank]\n");
      return;
   }

   while (*str && *str != ' ')
      *stack++ = *str++;
   if (*str)
      str++;
   strcpy(stack, " mailblock");
   stack = strchr(stack, 0);
   *stack++ = 0;
   message_string = stack;
   if (!strcasecmp("off", str)) {
      sprintf(stack, " [%s start%s getting your mails again.]\n", p->name, single_s(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 2);
   }
   if (!strcasecmp("on", str))
   {
      sprintf(stack, " [%s block%s you off from mailing %s.]\n",
         p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 1);
   }
   else {
      sprintf(stack, " [%s block%s you off from mailing %s.]\n",
         p->name, single_s(p), get_gender_string(p));
      stack = end_string(stack);
      actual_set_list(p, oldstack, 3);
   }
   stack = oldstack;
   message_string = 0;
}

/* prefer someone in multi tell matches */
void		prefer(player *p, char *str)
{
  char *oldstack;
  oldstack = stack;
  
  if(!*str) {
    tell_player(p, " Format: prefer <player(s)> on/off/[blank]\n");
    return;
  }
  
  while(*str && *str!=' ')
    *stack++ = *str++;
  if(*str)
    str++;
  strcpy(stack, " prefer");
  stack = strchr(stack, 0);
  *stack++=0;
  if(!strcasecmp("off", str)) {
    actual_set_list(p, oldstack, 2);
    stack = oldstack;
    return;
  }
  if(!strcasecmp("on", str)) {
    actual_set_list(p, oldstack, 1);
    stack = oldstack;
    return;
  }
  actual_set_list(p, oldstack, 3);
  stack = oldstack;
}


/* toggle whether iacga's are sent or not */
void            toggle_iacga(player * p, char *str)
{
   if (!strcasecmp("off", str))
   {
      p->saved_flags &= ~IAC_GA_ON;
      p->flags &= ~IAC_GA_DO;
   } else if (!strcasecmp("on", str))
   {
      p->saved_flags |= IAC_GA_ON;
      if (p->flags & EOR_ON)
    p->flags |= IAC_GA_DO;
   } else
   {
      p->saved_flags ^= IAC_GA_ON;
      p->flags ^= IAC_GA_DO;
      if (p->flags & EOR_ON)
    p->flags &= ~IAC_GA_DO;
   }

   if (p->saved_flags & IAC_GA_ON)
   {
      if (p->flags & IAC_GA_DO)
         tell_player(p, " You will get IAC GA controls after each prompt.\n");
      else
         tell_player(p, " You will get IAC GA's on prompts if IAC EOR is "
                        "unavailable.\n");
   } else
      tell_player(p, " You will not get IAC GA controls after each prompt.\n");
}


/* toggle earmuffs and blocktells on and off */

void            earmuffs(player * p, char *str)
{
   if (!strcasecmp("off", str))
      p->saved_flags &= ~BLOCK_SHOUT;
   else if (!strcasecmp("on", str))
      p->saved_flags |= BLOCK_SHOUT;
   else
      p->saved_flags ^= BLOCK_SHOUT;

   if (p->saved_flags & BLOCK_SHOUT)
      tell_player(p, " You are wearing earmuffs.\n");
   else
      tell_player(p, " You are not wearing earmuffs.\n");
}

/* toggle the pager on and off */

void            nopager(player * p, char *str)
{
   if (!strcasecmp("off", str))
      p->saved_flags &= ~NO_PAGER;
   else if (!strcasecmp("on", str))
      p->saved_flags |= NO_PAGER;
   else
      p->saved_flags ^= NO_PAGER;
   if (p->saved_flags & NO_PAGER)
      tell_player(p, " You will not get paged output.\n");
   else
      tell_player(p, " You will get paged output.\n");
}


void            blocktells(player * p, char *str)
{
   if (!strcasecmp("off", str))
      p->saved_flags &= ~BLOCK_TELLS;
   else if (!strcasecmp("on", str))
      p->saved_flags |= BLOCK_TELLS;
   else
      p->saved_flags ^= BLOCK_TELLS;

   if (p->saved_flags & BLOCK_TELLS)
      tell_player(p, " Tells to you will get blocked.\n");
   else
      tell_player(p, " Tells to you will not get blocked.\n");

}

/* toggle whether someone is hiding or not */


void            hide(player * p, char *str)
{

   if (!strcasecmp("off", str))
      p->saved_flags &= ~HIDING;
   else if (!strcasecmp("on", str))
      p->saved_flags |= HIDING;
   else
      p->saved_flags ^= HIDING;

   if (p->saved_flags & HIDING)
      tell_player(p, " You hide yourself away from prying eyes.\n");
   else
      tell_player(p, " People can find out where you are.\n");
}

/* toggle whether someone gets trans straight home */

void            straight_home(player * p, char *str)
{
   if (!strcasecmp("off", str))
      p->saved_flags &= ~TRANS_TO_HOME;
   else if (!strcasecmp("on", str))
      p->saved_flags |= TRANS_TO_HOME;
   else
      p->saved_flags ^= TRANS_TO_HOME;

   if (p->saved_flags & TRANS_TO_HOME)
      tell_player(p, " You will be taken to your home when you log in.\n");
   else
      tell_player(p, " You get taken to the entrance room when you log in.\n");
   strcpy(p->room_connect, "");
}

/* toggle see echo on and off */

void            see_echo(player * p, char *str)
{
   if (!strcasecmp("off", str))
      p->saved_flags &= ~SEEECHO;
   else if (!strcasecmp("on", str))
      p->saved_flags |= SEEECHO;
   else
      p->saved_flags ^= SEEECHO;

   if (p->saved_flags & SEEECHO)
      if (p->residency & (SU | ADMIN))
         tell_player(p, " You will now see who echos what.\n");
      else
         tell_player(p, " You will now see who echos what in public rooms.\n");
   else
      tell_player(p, " You will no longer see who echos what.\n");
}


/* see single entry */

void view_single_list(player * p, char *str)
{
   char *oldstack;
   list_ent *scan;
   int wibble = 0;

   oldstack = stack;
   if (p->saved)
   {
      for (scan = p->saved->list_top; scan && !wibble; scan = scan->next)
      {
         if (!strcasecmp(scan->name, str))
         {
            sprintf(stack, "Your list entry for '%s'\n\n%s\n", scan->name, list_header);
            stack = strchr(stack, 0);
            strcpy(stack, list_lines(scan));
            wibble = 1;
         }
      }
   } else
   {
      tell_player(p, " But you don't have a list!\n");
      stack = oldstack;
      return;
   }
   if (!wibble)
   {
      tell_player(p, " You dont have that name on your list ...\n");
      stack = oldstack;
      return;
   }
   stack = strchr(stack, 0);
   *stack++ = '\n';
   *stack++ = 0;
   tell_player(p, oldstack);
   stack = oldstack;
}

/* print out p2's list to p */

void            do_list(player * p, saved_player * sp, int count)
{
  list_ent       *l, *every = 0, *sus=0, *newbies=0, *friends = 0;
#ifdef ROBOTS
  list_ent	 *robots = 0;
#endif
  
  if (count)
    {
      sprintf(stack, "\n%s\n", list_header);
      stack = strchr(stack, 0);
      for (l = sp->list_top; l; l = l->next)
	{
	  if (!strcmp(l->name, "everyone"))
	    every=l;
#ifdef ROBOTS
	  else if (!strcmp(l->name, "robots"))
	    robots=l;
#endif
	  else if (!strcmp(l->name, "sus"))
	    sus=l;
	  else if (!strcmp(l->name, "newbies"))
	    newbies=l;
	  else if (!strcmp(l->name, "friends"))
	    friends = l;
	  else
	    {
	      strcpy(stack, list_lines(l));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	}
#ifdef ROBOTS
      if (every || sus || newbies || friends || robots)
#else
      if (every || sus || newbies || friends)
#endif
	{
	  strcpy(stack, "---------------------------------------------"
		 "-----------------------------\n");
	  stack = strchr(stack, 0);
#ifdef ROBOTS
          if (robots)
            {
              strcpy(stack, list_lines(robots));
              stack = strchr(stack, 0);
              *stack++ = '\n';
            }
#endif
	  if (sus)
	    {
	      strcpy(stack, list_lines(sus));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	  if (newbies)
	    {
	      strcpy(stack, list_lines(newbies));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	  if (friends)
	    {
	      strcpy(stack, list_lines(friends));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	  if (every)
	    {
	      strcpy(stack, list_lines(every));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	}
    }
}

/* view someone else's list, if you are Admin */

void            view_others_list(player * p, char *str)
{
   char           *oldstack, *arg2;
   int             count;
   player         *p2, dummy;
   list_ent       *l;

   oldstack = stack;
   memset(&dummy, 0, sizeof(player));

   if (!*str)
   {
      tell_player(p, " Format: vlist <player> [<entry>]\n");
      return;
   }

   arg2 = strchr(str, ' ');
   if (arg2)
   {
      *arg2++ = 0;
   }
   lower_case(str);
   p2 = find_player_global_quiet(str);
   if (!p2)
   {
      strcpy(dummy.lower_name, str);
      dummy.fd = p->fd;
      if (!load_player(&dummy))
      {
         tell_player(p, " That person does not exist!\n");
         return;
      }
      p2=&dummy;
   }

   if (!p2->saved)
   {
       tell_player(p, " That person has no list to view.\n");
       return;
   }
   count = count_list(p2);
   sprintf(stack, "%s is using %d of %s maximum %d list entries.\n",
      p2->name, count, their_player(p2), p2->max_list);
   stack = strchr(stack, 0);

   if (arg2)
   {
      l = find_list_entry(p2, arg2);
      if (l)
      {
         sprintf(stack, "\n%s\n", list_header);
         stack = strchr(stack, 0);
         strcpy(stack, list_lines(l));
         stack = strchr(stack, 0);
         *stack++ = '\n';
         *stack++ = '\0';
      } else
      {
         sprintf(stack, " %s has no list entry for '%s'.\n", p2->name, str);
         stack = end_string(stack);
      }
      tell_player(p, oldstack);
      stack = oldstack;
      return;
   }
   do_list(p, p2->saved, count);
   *stack++ = 0;
   pager(p, oldstack, 0);
   stack = oldstack;
}

/* view list */

void            view_list(player * p, char *str)
{
  char           *oldstack;
  list_ent       *l, *every = 0, *sus=0, *newbies=0, *friends=0;
#ifdef ROBOTS
  list_ent	 *robots=0;
#endif
  int             count;
  player         *p2 = 0;
  
  oldstack = stack;
  /*
   * This is the bit that means that an Admin doing 'list <person>' gets that
   * person's list, rather than just that person's entry on their own list.
   * Maybe something to stick in another command?
   */
  /*
   * if (*str && p->residency&ADMIN) p2=find_player_global(str);
    */
  if (*str)
    {
      view_single_list(p, str);
      return;
    }
  if (!p2)
    p2 = p;
  
  if (!p2->saved)
    {
      tell_player(p2, " You have no list to view.\n");
      return;
    }
  count = count_list(p2);
  sprintf(stack, " You are using %d of your maximum %d list entries.\n",
	  count, p2->max_list);
  stack = strchr(stack, 0);
  if (count)
    {
      sprintf(stack, "\n%s\n", list_header);
      stack = strchr(stack, 0);
      for (l = p2->saved->list_top; l; l = l->next)
	{
	  if (!strcmp(l->name, "everyone"))
	    every=l;
#ifdef ROBOTS
          else if (!strcmp(l->name, "robots"))
            robots=l;
#endif
	  else if (!strcmp(l->name, "sus"))
	    sus=l;
	  else if (!strcmp(l->name, "newbies"))
	    newbies=l;
	  else if (!strcmp(l->name, "friends"))
	    friends=l;
	  else
	    {
	      strcpy(stack, list_lines(l));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	}
#ifdef ROBOTS 
      if (every || sus || newbies || friends || robots)
#else
      if (every || sus || newbies || friends)
#endif
	{
	  strcpy(stack, "---------------------------------------------"
		 "-----------------------------\n");
	  stack = strchr(stack, 0);
#ifdef ROBOTS
	  if (robots)
	    {
	      strcpy(stack, list_lines(robots));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
#endif
	  if (sus)
	    {
	      strcpy(stack, list_lines(sus));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	  if (newbies)
	    {
	      strcpy(stack, list_lines(newbies));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	  if (friends)
	    {
	      strcpy(stack, list_lines(friends));
	      stack = strchr(stack, 0);
	      *stack++='\n';
	    }
	  if (every)
	    {
	      strcpy(stack, list_lines(every));
	      stack = strchr(stack, 0);
	      *stack++ = '\n';
	    }
	}
      
    }
  *stack++ = 0;
  pager(p, oldstack, 0);
  stack = oldstack;
}

/* Print out flags for the list entries */

char           *list_lines(list_ent * l)
{
   int             count;
   static char     string[80];
   string[0] = 0;
   sprintf(string, "%-18s-", l->name);
   for (count = 1; count < LIST_LINES_INT; count <<= 1)
   {
      if (l->flags & count)
    strcat(string, " YES");
      else
    strcat(string, "    ");
   }
   return string;
}


/* view list of someone else */


void            do_entry(player * p, player * p2)
{
  int             count;
  list_ent       *l;
  
  *stack++ = '\n';
  
  sprintf(stack, "%-14s", p2->name);
  stack = strchr(stack, 0);
  /*
  strcpy(stack, p2->name);
  for (count = MAX_NAME + 1; *stack; count--)
    stack++;
  for (; count > 3; count--)
    *stack++ = ' ';
  */
  if (!p2->saved)
    {
      strcpy(stack, "    - No list");
      stack = strchr(stack, 0);
      return;
    }

  l = find_list_entry(p2, p->lower_name);
  if (l)
    strcpy(stack, "    ");
#ifdef ROBOTS
  if (!l && p->residency & ROBOT)
    {
      l = find_list_entry(p2,"robots");
      if (l)
        strcpy(stack, "(R) ");
    }
#endif
  if (!l && p->residency & SU)
    {
      l = find_list_entry(p2,"sus");
      if (l)
	strcpy(stack, "(S) ");
    }
  if (!l && p->residency == NON_RESIDENT)
    {
      l = find_list_entry(p2,"newbies");
      if (l)
	strcpy(stack, "(N) ");
    }
  if (!l)
    {
      l = find_list_entry(p2, "everyone");
      if (!l)
	{
	  strcpy(stack, "    - No entry for you");
	  stack = strchr(stack, 0);
	  return;
	}
      strcpy(stack, "(E) ");
    }
  
  stack = strchr(stack, 0);
  *stack++ = '-';
  for (count = 1; count < LIST_LINES_INT; count <<= 1)
    {
      if (l->flags & count)
	strcpy(stack, " YES");
      else
	strcpy(stack, "    ");
      stack = strchr(stack, 0);
    }
}



void            check_entry(player * p, char *str)
{
   char           *oldstack, *text;
   player        **list, **scan;
   int             n, i;

   oldstack = stack;
   command_type = SEE_ERROR;

   if (!*str)
   {
      tell_player(p, " Format: check entry <player(list)>\n");
      return;
   }
   align(stack);
   list = (player **) ALFIX stack; 

   n = global_tag(p, str, 1);
   if (!n)
   {
      stack = oldstack;
      return;
   }
   text = stack;

   strcpy(stack, list_header);
   stack = strchr(stack, 0);
   /* list_header has a \n on, we dont want it here, so */
   stack-=1;
   *stack = 0;
   for (scan = list, i = 0; i < n; i++, scan++)
      if (*scan != p)
        do_entry(p, *scan);
   *stack++ = '\n';
   *stack++ = 0;
   cleanup_tag(list, n);
   pager(p, text, 0);
   stack = oldstack;
}


/* see if a particular player can recieve a particular message */

int             test_receive(player * p)
{
  int             cerror = 1;
  list_scan ls;
  char           *oldstack;
  
  oldstack = stack;
  
  if (!current_player)
    return 1;
  if (p == current_player)
    return 1;
  if (current_player->residency & ADMIN && command_type & PERSONAL &&
      !(sys_flags & EVERYONE_TAG || command_type & FRIEND_COM))
    return 1;
  
  if (command_type & EMERGENCY)
    return 1;
  
  if (command_type & PERSONAL && !(sys_flags & EVERYONE_TAG))
  {
      if (command_type & WARNING)
	  return 1;

      ls = listscan(current_player, p);
        
      if (ls.l && (ls.l->flags & IGNORE || ls.l->flags & BLOCK || ls.lf->flags & IGNORE || ls.lf->flags & BLOCK) &&
        !(ls.l->flags & NOISY || ls.lf->flags & NOISY))
      {
	if (command_type & SEE_ERROR)
	{ 
	   if(ls.l->flags & IGNORE)
	     vtell_current("You can't get to %s because you "
		      "are ignoring %s.\n", p->name, get_gender_string(p));
	   else
	     vtell_current("You can't get to %s because you "
	              "are blocking %s.\n", p->name, get_gender_string(p));
	}
	sys_flags |= FAILED_COMMAND;
	stack = oldstack;
	return 0;
      }
  }
  ls = listscan(p, current_player);
  
  if ((p->saved_flags & BLOCK_TELLS) && !(ls.l && (ls.l->flags & NOISY || ls.lf->flags & NOISY)) &&
      (command_type & PERSONAL))
  {
    if (command_type & SEE_ERROR)
      vtell_current("%s foils you with a blocktell.\n", p->name);
    sys_flags |= FAILED_COMMAND;
    stack = oldstack;
    return 0;
  }
  
  /* friends stuff */
  if(command_type & FRIEND_COM) {
    if(p->sflags & BLOCK_FRIEND && !(ls.l && (ls.l->flags & NOISY || ls.lf->flags & NOISY))) {
      if(current_player->sflags & LIST_FAILS) 
        vtell_current(" [%s is blocking friend tells/remotes/etc.]\n", p->name);
      stack = oldstack;
      return 0;
    }
    if(command_type & TFREPLY && p->sflags & BLOCK_TF_REPLIES && 
     !(ls.l && (ls.l->flags & NOISY || ls.lf->flags & NOISY))) {
      if(current_player->sflags & LIST_FAILS) 
        vtell_current(" [%s is blocking friend replies.]\n", p->name);
      stack = oldstack;
      return 0;
    }
    if(ls.l && (ls.l->flags & FRNDBLOCK) && !(ls.l->flags & NOISY || ls.lf->flags & NOISY)) {
      if(current_player->sflags & LIST_FAILS) 
        vtell_current(" [%s is blocking friend tells/remotes/replies/etc from you.]\n", p->name);
      stack = oldstack;
      return 0;
    }
  }  
  
  /* thanks to jonathan for reminding me of this bug */
  if (p->location != current_player->location)
  {    
      if (p->saved_flags & BLOCK_SHOUT && (command_type & EVERYONE || sys_flags & EVERYONE_TAG))
	return 0;
  } 
  else
  {
      if ( p->saved_flags & BLOCK_SHOUT && command_type & EVERYONE )
	return 0;
  }
  if (!ls.l)
    return 1;
  
  if ((ls.l->flags & BLOCK || ls.lf->flags & BLOCK) && !(ls.l->flags & NOISY || ls.lf->flags & NOISY)
      && command_type & PERSONAL)
    {
      if (command_type & SEE_ERROR)
	vtell_current("You are blocked by %s.\n", p->name);
      sys_flags |= FAILED_COMMAND;
      stack = oldstack;
      return 0;
    }
  if (sys_flags & ROOM_TAG || sys_flags & EVERYONE_TAG
      || sys_flags & FRIEND_TAG)
    cerror = 0;
  
  if ((ls.l->flags & IGNORE || ls.lf->flags & IGNORE) && !(ls.l->flags & NOISY || ls.lf->flags & NOISY))
    {
      if (command_type & SEE_ERROR && cerror) {
	if ((int)strlen(p->ignore_msg) > 0)
          vtell_current("%s\n", p->ignore_msg);
	else
          vtell_current("You are ignored by %s.\n", p->name);
      }
      sys_flags |= FAILED_COMMAND;
      stack = oldstack;
      return 0;
    }
  stack = oldstack;
  return 1;
}


/* new verson of do inform i wrote for surfers ;) */
void do_inform(player *p, char *msg)
{
    player *scan;
    char *oldstack;
    list_ent *l, *s;
    int type;
    
    oldstack = stack;
    
    if ( !(p->location) || !(p->name[0]) )
	return;
    
    for (scan = flatlist_start; scan; scan = scan->flat_next) {
	if ((scan != p) && !(scan->flags & PANIC) &&
	    (scan->name[0]) && (scan->location)) {
	  type = 0;
	  if (p->residency & PSU) {
            s = find_list_entry(scan, "sus");
	    if (s && (s->flags & INFORM))
	      type |= iINF_SU;
	  }
	    
	  l = find_list_entry(scan, p->lower_name);
#ifdef ROBOTS
	  if (!l && (p->residency & ROBOT_PRIV)) {
	    l = find_list_entry(scan, "robots");
	    if (l && (l->flags & INFORM))
	      type |= iINF_ROBOT;
	  }
#endif
	  if (!l && (p->residency & PSU))
	    l = find_list_entry( scan, "sus" );
	  if (!l && (p->residency == NON_RESIDENT)) {
	    l = find_list_entry(scan, "newbies");
            if (l && (l->flags & INFORM))
	      type |= iINF_NEWBIE;
	  }
	  if (!l) {
	    l = find_list_entry(scan, "everyone");
	    command_type = LIST_EVERYONE;
	  }
	  if (l && (l->flags & FRIEND)) {
	    s = find_list_entry( scan, "friends" );
	    if ((s && (s->flags & INFORM)) || (l->flags & INFORM))
	      type |= iINF_FRIEND;
	    if ( (s && (s->flags & BEEP)) || (l->flags & BEEP))
	      type |= iBEEP_FRIEND;
	  }
	  
	  if (l && (l->flags & INFORM || type)) {
	    command_type = 0;
	    if (scan->residency & (TRACE | SU | ADMIN))
	      sprintf(stack, msg, p->name, p->inet_addr);
            else
	      sprintf(stack, msg, p->name, "");
	    stack = strchr(stack, 0);
	    if (type & iINF_SU) {
	      if (p->residency & ADMIN)
		sprintf( stack, " [Admin]" );
              else if (p->residency & LOWER_ADMIN)
                sprintf( stack, " [Lower Admin]" );
              else if (p->residency & SU)
		sprintf( stack, " [Superuser]" );
	      else if (p->residency & PSU)
		sprintf( stack, " [Pseudo SU]" );
              stack=strchr(stack,0);
            }
            if (type & iINF_FRIEND) {
	      sprintf( stack, " [Friend]" );
	      stack=strchr(stack,0);
            }		
#ifdef ROBOTS
            if (type & iINF_ROBOT) {
              sprintf(stack, " [Robot]" );
              stack = strchr(stack, 0);
            }
#endif
            if (type & iINF_NEWBIE) {
	      sprintf( stack, " [Newbie]" );
              stack=strchr(stack,0);
            }
	    if (l->flags & BEEP || type & iBEEP_FRIEND)
	      *stack++ = 7;
	    *stack++ = '\n';
	    *stack++ = 0;
	    if (l->flags & FRIEND) 
	      command_type |= (HIGHLIGHT|PERSONAL|WARNING);
	    tell_player(scan, oldstack);
	    command_type = 0;
	    stack = oldstack;
	  }
        }
    }
}



/* toggle what someone sees */

void            toggle_tags(player * p, char *str)
{
   char           *oldstack;
   int             change;

   oldstack = stack;
   if (!*str)
   {
      change = p->saved_flags
                & (TAG_ECHO | TAG_PERSONAL | TAG_ROOM | TAG_SHOUT | TAG_AUTOS);
      if (!change)
    tell_player(p, " You have no show flags enabled.\n");
      else
      {
    strcpy(stack, " You will get shown : ");
    stack = strchr(stack, 0);
    if (change & TAG_ECHO)
    {
       strcpy(stack, "echos, ");
       stack = strchr(stack, 0);
    }
    if (change & TAG_PERSONAL)
    {
       strcpy(stack, "tells to you, ");
       stack = strchr(stack, 0);
    }
    if (change & TAG_ROOM)
    {
       strcpy(stack, "room commands, ");
       stack = strchr(stack, 0);
    }
    if (change & TAG_SHOUT)
    {
       strcpy(stack, "shouts, ");
       stack = strchr(stack, 0);
    }
    if (change & TAG_AUTOS)
    {
       strcpy(stack, "autos, ");
       stack = strchr(stack, 0);
    }
    *stack = 0;
    *(--stack) = '\n';
    *(--stack) = '.';
    stack = end_string(stack);
    tell_player(p, oldstack);
      }
      stack = oldstack;
      return;
   }
   change = get_flags(p, str, show_flags);
   if (!change)
   {
      tell_player(p, " Bad flag list, no changes made. \n"
                     " Possible flags are echo,tell,room,shout,autos\n");
      return;
   }
   p->saved_flags ^= change;
   tell_player(p, " Show flags entries toggled.\n");
}


/*
 * checks whether str is a person on the program (inc newbie) or if they are
 * in the playerfiles  Returns their uppercase name
 */
char           *check_legal_entry(player * p, char *str, char *spang)
{
   char           *oldstack, *store;
   static char     wibble[20];
   player         *plyr;
   saved_player   *splyr;
   int		   fscan = 0, found = 0;
   
#ifdef INTERCOM
   if (*str && (int)strlen(str) > (MAX_NAME-1))
     str[MAX_NAME-1]=0;
#endif    
  
   oldstack = stack;
   lower_case(str);
   if((store = strchr(str, ' ')))
     *store = 0;

   if(!strcasecmp(str, p->name)) {
      tell_player(p, " You can't set a list entry for yourself!\n");
      return 0;
   }
   
   if (!strcasecmp(str, "newbies")) {
      strcpy(wibble, "newbies");
      return wibble;
   }
   
   if (!strcasecmp(str, "sus") && p->residency & SU) {
      strcpy(wibble, "sus");
      return wibble;
   }
   
#ifdef ROBOTS
   if(!strcasecmp(str, "robots")) {
     strcpy(wibble, "robots");
     return wibble;
   }
#endif
   
   if (!strcasecmp(str, "everyone")) {
      strcpy(wibble, "everyone");
      return wibble;
   }
   
   if(!strcasecmp(str, "friends")) {
      store = spang;
      while(*store) {
         /* copy the users input onto the stack */
         for(stack = oldstack; *store && *store!=','; *stack++ = *store++);
         *stack++=0;
         /* if *store is left on a comma, move it along */
         if(*store && *store==',') store++;
         /* now we have the flag to check in oldstack, do a pass for it */
         found=0;
         for(fscan=0; fscan<ALLOWED_FRIEND; fscan++) 
           if(!strcasecmp(allowed_friend[fscan], oldstack))
             found=1;
         if(!found) {
           vtell_player(p, " '%s' is not a valid flag to set for 'friends'.\n", oldstack);
           stack = oldstack;
           return 0;
         }
      }
      stack = oldstack;
      strcpy(wibble, "friends");
      return wibble;
   }
   
#ifdef INTERCOM
   if(strchr(str, '@'))
   {
     strcpy(wibble, str);
     return wibble;
   }
#endif
   
   if ((int)strlen(str) > 20)
   {
      vtell_player(p, " Too long a name '%s'\n", str);
      return 0;
   }
   if ((plyr = find_player_absolute_quiet(str)))
   {
      strcpy(wibble, plyr->name);
      return wibble;
   }
   if (!(splyr = find_saved_player(str)))
   {
      vtell_player(p, " There is no such person '%s'\n", str);
      return 0;
   }
   if (!splyr)
   {
      log("error", "Spooned again chris. check_legal_entry");
      return 0;
   }
   if ((splyr->residency == STANDARD_ROOMS))
   {
      vtell_player(p, " Sorry, %s is a system name.\n", str);
      return 0;
   }
   if (splyr->residency & BANISHD)
   {
      if (splyr->residency == BANISHD)
         vtell_player(p, " Sorry, %s has been banished. (Name Only)\n", str);
      else
         vtell_player(p, " Sorry, %s has been banished.\n", str);
      return 0;
   }
   strcpy(wibble, splyr->lower_name);
   return wibble;
}


/*
 * Need some stuff to delete entries for newbies who have logged off and to
 * wipe newbies from residents lists when they log off
 */

/*
 * This one is for when a resident logs off... all the newbies are stripped
 * from their list
 */

void            check_list_resident(player * p)
{
   player         *scan;
   list_ent       *l;

   for (scan = flatlist_start; scan; scan = scan->flat_next)
   {
      /* PARDON?  Search for an entry on every person before newbie
         status??  This is now fixed. */
      if(scan->residency==NON_RESIDENT) {
        l = find_list_entry(p, scan->lower_name);
        if (l)
    	  delete_entry(p->saved, l);
      }
   }
}


/*
 * Now to strip all the entries of a newbie, when (s)he logs off
 */
void            check_list_newbie(char *lowername)
{
   player         *scan;
   list_ent       *l;

   for (scan = flatlist_start; scan; scan = scan->flat_next)
   {
      l = find_list_entry(scan, lowername);
      if (l)
    delete_entry(scan->saved, l);
   }
}


void            check_alist(player * p, char *str)
{
   char           *oldstack;
   int             count;
   list_ent       *l;
   player         *p2, dummy;

   oldstack = stack;
   memset(&dummy, 0, sizeof(player));

   if (!*str)
   {
      p2 = p;
      if (p->saved)
      {
    count = count_list(p);
    sprintf(stack, "You are using %d of your maximum %d list entries.\n",
       count, p->max_list);
    stack = strchr(stack, 0);
      } else
      {
    tell_player(p, " You have no list to check.\n");
    return;
      }
   } 

   else if (p->residency & (ADMIN | LOWER_ADMIN))
   {
       lower_case(str);
       p2 = find_player_global_quiet(str);
       if (!p2)
       {
	   strcpy(dummy.lower_name, str);
	   dummy.fd = p->fd;
	   if (!load_player(&dummy))
	   {
	       tell_player(p, " That person does not exist!\n");
	       return;
	   } else
	       p2 = &dummy;
       }
       if (!p2->saved)
       {
	   tell_player(p, " That person has no list to check.\n");
	   return;
       }
       count = count_list(p2);
       sprintf(stack, " %s %s using %d of %s maximum %d entries.\n",
	       p2->name, isare(p2) ,count, their_player(p2), p2->max_list);
       stack = strchr(stack, 0);
   } else
   {
       tell_player(p, " You can't do that!\n");
       return;
   }
   
   
   for (l = p2->saved->list_top; l; l = l->next)
       if (!strcmp(l->name, "everyone"))
       {
	   strcpy(stack, list_header);
	   stack = strchr(stack, 0);
	   strcpy(stack, list_lines(l));
	   stack = strchr(stack, 0);
	   *stack++ = '\n';
       }
   *stack++ = 0;
   tell_player(p, oldstack);
   stack = oldstack;
}

void blank_list(player *p, char *str)
{
  if (!p->saved)
    {
      tell_player(p, " You do not have a list to alter, "
		  "since you have no save file\n");
      return;
    }
  
  /*Null the head of the list*/
  /*DONT free the other list, cos the reason the list is being blanked
    is probably due to a mislaid pointer and we dont want to go about
    deleting other useful memory*/
  p->saved->list_top=0;
  
  tell_player(p," List deleted\n");

  return;
}


/* command to toggle friend blocking */
void	block_friends(player *p, char *str)
{
  if(!strcasecmp("off", str))
    p->sflags &= ~BLOCK_FRIEND;
  else if(!strcasecmp("on", str))
    p->sflags |= BLOCK_FRIEND;
  else
    p->sflags ^= BLOCK_FRIEND;
    
  if(p->sflags & BLOCK_FRIEND) {
    tell_player(p, " You block friend tells.\n");
    p->tfreplytime = time(0);
  }
  else
    tell_player(p, " You see friend tells again.\n");
}


void		toggle_listing_of_tf(player *p, char *str)
{
  if(!strcasecmp("off", str))
    p->sflags &= ~LIST_FAILS;
  else if(!strcasecmp("on", str))
    p->sflags |= LIST_FAILS;
  else
    p->sflags ^= LIST_FAILS;
    
  if(p->sflags & LIST_FAILS)
    tell_player(p, " You see who blocks your friend messages.\n");
  else
    tell_player(p, " You do not see who blocks your friend messages.\n");
}


/* and now a way to remove yourself from another persons list */
void		rm_list(player *p, char *str)
{
  player *p2;
  int change = 0;
  list_ent *l;
  
  if(!*str) {
    tell_player(p, " Format: rm_list <player(s)>\n");
    return;
  }
  
  p2 = find_player_global(str);
  if(!p2)
    return;
    
  /* got the player to remove from now */
  if(!p2->saved || !p2->saved->list_top) {
    tell_player(p, " That person does not have a list.\n");
    return;
  }
  
  /* find our entry on their list */
  l = find_list_entry(p2, p->lower_name);
  if(!l) {
    tell_player(p, " You are not on their list!\n");
    return;
  }
  
  /* compse flaglist */
  if(p->residency & ADMIN) /* muahahahahah - spoons */
    change = get_flags(p2, "noisy,ignore,inform,grab,friend,bar,invite,beep,block,key,find,friendblock,mailblock", list_flags);
  else
    change = get_flags(p2, "noisy,grab,friend,invite,key,find", list_flags);
    
  /* oh dear */
  if(!change) {
    tell_player(p, " Bad flag list, no changes made.\n");
    return;
  }
  
  /* sort it. */
  l->flags &= ~change;
  if(!l->flags)
    delete_entry(p2->saved, l);
  /* tell user */
  vtell_player(p, " Your entry on %s'%s list has been wiped.\n", p2->name, trailing_s(p2));
}


/* kills dead entries :-) */
void 		purge_list(player *p, char *str)
{
   char	*oldstack;
   saved_player	*sp, *sp2;
   int	count = 0;
   list_ent *l, *spang;
   char name[MAX_NAME];
   
   oldstack = stack;
   sp = p->saved;
   if (!sp)
   {
      tell_player(p, " You have no saved data, and therefore no list.\n");
      return;
   }
   if (!sp->list_top)
   {
      tell_player(p, " You don't seem to have a list to purge!\n");
      return;
   }
   sprintf(stack, " Names deleted from list:");
   stack = strchr(stack, 0);
   l = sp->list_top;
   while(l)
   {
      strncpy(name, l->name, MAX_NAME);
      lower_case(name);
      sp2 = find_saved_player(name);
      if ((!sp2 || (sp2->residency == BANISHD)) &&
          (strncasecmp("sus", name, MAX_NAME) && 
#ifdef ROBOTS
	   strncasecmp("robots", name, MAX_NAME) &&
#endif
           strncasecmp("newbies", name, MAX_NAME) &&
           strncasecmp("friends", name, MAX_NAME) &&
           strncasecmp("everyone", name, MAX_NAME)))
      {
         sprintf(stack, " %s,", l->name);
         stack = strchr(stack, 0);
         spang = l;
         l = l->next;
         delete_entry(sp, spang);
         count++;
      }
      else
         l = l->next;
   }
   
   if (count == 0)
      tell_player(p, " No entries removed.\n");
   else
   {
      stack--;
      sprintf(stack, ".\n");
      stack = end_string(stack);
      tell_player(p, oldstack);
   }
   stack = oldstack;
}

