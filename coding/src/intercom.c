...
static char *end_string(char *str)
{
  return (strchr(str,0)+1);
}

static int lower_case(char *str)
{
  while (*str)
    {
      *str=tolower(*str);
...
static void add_new_jobs(void)
{
  ...
  for (loopa=0;loopa<10;loopa++)
    {
      new_job=(job_list *)MALLOC(sizeof(job_list));
...
{
  job_anchor=(job_list *)MALLOC(sizeof(job_list));
  free_jobs_anchor=(job_list *)MALLOC(sizeof(job_list));
...
static void make_new_packets(void)
{
  ...
  for (loop=0;loop<10;loop++)
    {
      new_var=(packet *)MALLOC(sizeof(packet));
...
{
  free_packet_anchor=(packet *)MALLOC(sizeof(packet));
...
static void parse_reply_from_remote(talker_list *remote_talker,char *str)
{
  ...
  switch(*str)
    ...
    case TALKER_IGNORED:
      strcpy(current_name,this_job->sender);
      tell_personal(" %s is ignoring %s.",this_job->target,TALKER_NAME);
      break;
      
    case TALKER_BLOCKED:
      strcpy(current_name,this_job->sender);
      tell_personal(" %s is blocking %s.",this_job->target,TALKER_NAME);
      break;
...
static void get_remote_list(talker_list *remote_talker,char *str)
{
  ...
  if (!strcasecmp(name,TALKER_NAME) || 
...
static int get_new_talker_connection(void)
{
  ...
  new_var=(talker_list *)MALLOC(sizeof(talker_list));
  scan->next=new_var;
  new_var->fd=new_fd;
...
static void send_hello(talker_list *remote_talker)
{
  tell_remote_talker(remote_talker,"%c%s:%s:%d",HELLO_I_AM,
		     TALKER_NAME,INTERCOM_ABBR,portnumber);
...
static void add_new_server_to_file(char *str)
{
  ...
  *abbr=':';
  ...
  fp=fopen("files/stuff/intercom.dbase","a");
  if (fp)
    {
      fprintf(fp,"%s\n",name);
      fclose(fp);
...
static void read_talker_database(void)
{
  ...
  talker_list_anchor = (talker_list *)MALLOC(sizeof(talker_list));
  validation_anchor = (talker_list *)MALLOC(sizeof(talker_list));

  fp=fopen("files/stuff/intercom.dbase","r");
  if (fp)
    {
      while (!feof(fp))
	{
...
static void show_all_links(int hidden,int up_only)
{
  ...
  buffer=(char *)MALLOC(strlen(oldstack)+2);
...
static void close_all_links(void)
{
  ...
  if (!(intercom_status & BAR_ALL_CONNECTIONS))
    {
      tell_personal(" You close %s to external messages.",TALKER_NAME);
...
static void open_all_links(void)
{
  ...
  if (intercom_status & BAR_ALL_CONNECTIONS)
    {
      tell_personal(" You open %s to external messages.",TALKER_NAME);
...
void sync_talkers()
{
  ...
  fp=fopen("files/stuff/intercom.dbak","w");
  if (!fp)
    {
  ...
  rename("files/stuff/intercom.dbak","files/stuff/intercom.dbase");
...
static void request_stats(char *str)
{
  ...
  else if (!strcasecmp(str,TALKER_NAME) || !strcasecmp(str,INTERCOM_ABBR))
    {
      stats = &server_net;
...
extern int main(int argc,char **argv)
{
  ...
  stack_start = (char *)MALLOC(INTERCOM_STACK);
  stack = stack_start;

  log("intercom","Intercom booting");

  if (strchr(TALKER_NAME,':'))
    {
      log("intercom","TALKER_NAME may NOT have a : in it. Not booting.");
      exit (-1);
    }
